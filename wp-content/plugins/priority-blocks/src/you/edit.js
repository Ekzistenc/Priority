...
export default function Edit( { attributes, setAttributes } ) {
	const { title, youList, image_url, image_alt, image_id, video } =
		attributes;
			...
			<InspectorControls>
				<PanelBody title="Настройки секции YOU">
					...
					<SNDMediaUpload
						...
						labelButton={ `${
							video?.poster?.url ? 'Изменить' : 'Выбрать'
						} постер` }
					/>
					<hr />
			...
			<section { ...useBlockProps( { className: 'sd-you' } ) }>
				<div className="container">
					...
					<div className="sd-you__wrapper">
						{ video?.url ? (
							<a class="glightbox5" href={ video.url }>
								{ video?.poster?.url ? (
									<img
									...
									<div className="sd-you__poster-placeholder"></div>
								) }
							</a>
						) : (
							<div />
